...
const MARGIN = 40;
const TOTAL_QUESTIONS = STABILITAS_QUIZ_QUESTIONS.length;
...
/**
 * Kuis Stabilitas — a dedicated quiz scene (parallel to, but independent
 * from, AnatomiStruktur's QuizScene) since this task's interaction model
 * differs: picking an option immediately reveals correct/incorrect plus a
 * pembahasan panel, and the result screen has a per-question review
 * checklist — none of which the generic QuizScene supports, and that scene
 * is already shared by the Anatomi quiz and the global final evaluation, so
 * it's left untouched rather than bent to fit this flow.
 */
export class StabilitasQuiz extends Scene {
    ...
    private questions = STABILITAS_QUIZ_QUESTIONS;
    ...
    create() {
        ...
        this.questions = shuffleQuestions(STABILITAS_QUIZ_QUESTIONS);
    ...
    private renderResult() {
        ...
        const buttonHeight = 48;
        const buttonsTop = listTop + TOTAL_QUESTIONS * rowHeight + 30;
        const gap = 16;
